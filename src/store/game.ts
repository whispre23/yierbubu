...
// console.log(v);
    ...
    // const levelBoardDom:any = uni.createSelectorQuery().select('.level-board');
    // const levelBoardDom:any = uni.createSelectorQuery().select("#gamePage")

    // console.log(levelBoardDom);
    // levelBoardDom._selectorQuery._page.$el.style.width = widthUnit * boxWidthNum + "px";
    // levelBoardDom._selectorQuery._page.$el.style.height = heightUnit * boxHeightNum + "px";
    ...
    console.log("块数单位-类型数量*几块可以合成的数量", blockNumUnit);
    ...
    console.log("随机区域生成的总块数", totalRandomBlockNum);

    // 需要的最小块数
    const minBlockNum =
      gameConfig.levelNum * gameConfig.levelBlockNum + totalRandomBlockNum;
    console.log("加上随机区域的总体需要的最小块数", minBlockNum);
    ...
    // 需要用到的动物数组
    const needAnimals = _.shuffle(gameConfig.pictures).slice(0, gameConfig.typeNum);
    // 依次把块塞到数组里
   ...
   * @param block
   */
  const genLevelRelation =  (block: BlockType,back=false) => {
    // console.log(block);
    if(back){
      // block.level += 1;
  ...
  const doClickBlock = (block: BlockType, randomIdx = -1, force = false) => {
    ...
    console.log('点击详情',block,randomIdx,force);
    console.log('currSlotNum.value',currSlotNum.value);
    console.log('gameConfig.slotNum ',gameConfig.slotNum );
    console.log('block.status',block.status);
    console.log('block.lowerThanBlocks.length',block.lowerThanBlocks.length);
    ...
    if (randomIdx >= 0) {
      ...
      // 移除覆盖关系
      console.log('opHistory',opHistory);
      console.log('block',block);
      
      block.higherThanBlocks.forEach((higherThanBlock) => {
        // console.log('higherThanBlock',higherThanBlock);
        
        _.remove(higherThanBlock.lowerThanBlocks, (lowerThanBlock) => {
          // console.log('lowerThanBlock',lowerThanBlock);
  ...
  const doStart = () => {
    ...
    console.log('levelBlocks', levelBlocks);
    console.log('randomBlocks', randomBlocks);
    console.log( 'slotArea', slotArea);
  ...
  const doRevert = async() => {
    console.log('opHistory',opHistory);
    console.log('slotAreaVal',slotAreaVal);
    console.log('currSlotNum',currSlotNum);
    ...
    await genLevelRelation(opHistory[opHistory.length - 1],true);
    // @ts-ignore
    slotAreaVal.value[currSlotNum.value - 1] = null;
    opHistory.pop()
    currSlotNum.value= currSlotNum.value - 1
  };
